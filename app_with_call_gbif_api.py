...
def load_tdwg_regions(path_tdwg_maps):
    ...
    with open(path_tdwg_maps+"/level4.geojson") as geojsonfile:
        ...
        tdwg_level4['Level4_cod'] = tdwg_level4['Level3_cod'] + \
            tdwg_level4['Level4_2']
    tdwg_data = dict()
...
def load_mapping_tdwg_eco_regions(path_data):
    mapping_level4_df = pd.read_csv(
        path_data+"mapping_tdwg_level4_eco_regions.csv")
    mapping_data = dict()
...
def panel_choix_genus(debug_mode):
    with st.container():
        ...
        selected_genus = st.selectbox(
            label='Genus', index=idx, options=st.session_state.list_genus, key='genus')
        if (debug_mode):
            st.write(selected_genus)
        st.session_state.list_species = list(
            set(flow_df[flow_df['genus'] == st.session_state['genus']]['species'].values))
    ...
    with st.container():
        with st.form('species gbif selection'):
            if 'species' not in st.session_state:
                ...
                try:
                    idx = st.session_state.list_species.index(
            ...
            if debug_mode:
                ...
                st.write(idx)
            searched_gbif_name = st.selectbox(
                label='Species', index=idx, options=st.session_state.list_species, key='other')
            ...
            if submitted_gbif:
                ...
                name_backbone, dict_results = mf.search_gbif_from_name_and_rank(
                    searched_gbif_name, rank)
                # Construction du geodataframe avec les résultats de la requête au GBIF
                gbif_occ_df = mf.build_geo_df(
                    dict_results, features_to_keep, eco_regions_df.crs)

                if (gbif_occ_df.size == 0):
                    body = "No occurrence of " + searched_gbif_name + \
                        " with coordinates was found in the GBIF database"
                    st.warning(body, icon="😢")
                else:
                    if debug_mode:
                        st.markdown(set(gbif_occ_df['basisOfRecord']))
                    # On cherche les eco-regions pour les occurrences GBIF
                    eco_regions_gbif_found_df = mf.find_eco_regions(
                        gbif_occ_df, eco_regions_df)
                    # On cherche maintenant les phyto-regions du TDWG level 4
                    tdwg_regions_gbif_found_df = mf.find_phyto_regions(
                        gbif_occ_df, tdwg_data['tdwg_level4'])
...
def panel_flow_choix_species(debug_mode):
    ...
    with st.container():
        tdwg_regions_flow = ff.find_phyto_regions_for_flow_species(
            st.session_state['species'], flow_df)
        true_columns_dict, eco_regions_list = ff.get_level4_eco_id_list(
            mapping_data['tdwg_level4'], tdwg_regions_flow)
...
def panel_gbif_comment(gbif_occ_df, eco_regions_gbif_found_df, tdwg_regions_gbif_found_df, debug_mode):
    ...
    eco_regions_found_data = eco_regions_gbif_found_df['ECO_NAME'].value_counts(
    ).reset_index().rename(columns={'index': 'Name', 'ECO_NAME': 'Number of occurrences'})
    if debug_mode:
        st.dataframe(eco_regions_found_data)
    line1 = st.session_state['species'] + " has been found " + \
        str(len(gbif_occ_df)) + " times in the GBIF database"
   ...
   # line3 = eco_regions_found_data['Name'].unique()
    ...
    tdwg_regions_gbif_found_data = tdwg_regions_gbif_found_df['Level_4_Na'].value_counts(
    ).reset_index().rename(columns={'index': 'Name', 'Level_4_Na': 'Number of occurrences'})
...
def panel_flow_comment(eco_regions_flow_found_df):
    ...
    line1 = st.session_state.searched_name + " has been found " + \
        str(eco_regions_flow_found_df.shape[0]) + " times in the FLOW database"
...
def show_gbif_map(tdwg_regions_gbif_found_df, eco_regions_found_df, geo_occ_df):
    map = mf.create_map_for_gbif_occurrences(
        tdwg_regions_gbif_found_df, eco_regions_found_df, geo_occ_df)
    with st.container():
...
def show_flow_map(level4_cod_values, eco_id_list):
    map = ff.show_map(tdwg_data, eco_regions_df,
                      level4_cod_values, eco_id_list)
    with st.container():
        map_html = map._repr_html_()
        st.components.v1.html(map_html, height=900, width=1000)
...
def hc_header():
    ...
    st.header('Auchenorrhyncha | **:green[Fulgoromorpha]** | Fulgoroidea | Cixiidae',
              help='https://fr.wikipedia.org/wiki/Cixiidae')
    st.write('-----------------')
...
def hc_sidebar():
    ...
    image = Image.open(
        'images/Tachycixius venustulus (Streifen-Glasflügelzikade)M1.2.jpg')
    st.sidebar.image(image, caption='Tachycixius venustulus')
    image = Image.open(
        'images/Pentastiridius leporinus (Schilf-Glasflügelzikade)W1.2.jpg')
...
def hc_body(debug_mode):
    ...
    with tab1:
        ...
        if (eco_regions_gbif_found_df.size != 0):
            panel_gbif_comment(geo_gbif_occ_df, eco_regions_gbif_found_df,
                               tdwg_regions_gbif_found_df, debug_mode)
            show_gbif_map(tdwg_regions_gbif_found_df,
                          eco_regions_gbif_found_df, geo_gbif_occ_df)
    with tab2:
        # Flow
        ...
        if 'species' not in st.session_state:
            ...
            line = "Here are the locations of occurences of :red[" + \
                sp + "] documented in FLOW (TDWG)"
            st.markdown(line)

            tdwg_regions_flow, eco_regions_list, flow_occ_df = panel_flow_choix_species(
                debug_mode)
