    ...
    with st.container() :
        with st.form('species selection'):
            ...
            if submitted :
                ...
                flow_occ_df, eco_regions_flow_found_df = mf.find_eco_regions_for_flow_species(flow_df[flow_df['species']== searched_name], eco_regions_df)
        ...
        return eco_regions_flow_found_df, eco_regions_gbif_found_df, geo_occ_df, flow_occ_df

def panel_gbif_comment(eco_regions_gbif_found_df, geo_occ_df):
    ...
    return eco_regions_gbif_found_df,geo_occ_df


def panel_flow_comment(eco_regions_flow_found_df, flow_occ_df):
...
def hc_body():
    ...
    with tab1:
        ...
        eco_regions_flow_found_df, eco_regions_gbif_found_df, geo_occ_df,flow_occ_df = panel_choix_species(debug_mode,genus)
        if (eco_regions_gbif_found_df.size != 0) : 
            panel_gbif_comment(eco_regions_gbif_found_df, geo_occ_df)
            show_map(eco_regions_gbif_found_df, geo_occ_df)
        if (eco_regions_flow_found_df.size != 0): 
            panel_flow_comment(eco_regions_flow_found_df, flow_occ_df)
